...
new_method = """    def generate_report(self, payload):
        ...
        synthesis = payload.get('research_synthesis', {}).get('synthesis', {})
        ...
        md += f"## 🔬 Policy Research Synthesis\\n\\n"
        md += f"### Executive Summary\\n{synthesis.get('executive_summary', 'No summary generated.')}\\n\\n"
        
        md += f"### Key Policy Trends\\n"
        for t in synthesis.get('policy_trends', []): md += f"* {t}\\n"
        
        md += f"\\n### Top Risks Identified\\n"
        for r in synthesis.get('top_risks', []): md += f"* {r}\\n"
        
        md += f"\\n### Recommended Monitoring Actions\\n"
        for a in synthesis.get('recommended_monitoring_actions', []): md += f"* {a}\\n"
        
        md += f"\\n### Jurisdictions to Watch\\n"
        for j in synthesis.get('jurisdictions_to_watch', []): md += f"* {j}\\n"
        
        md += f"\\n## 🧠 Accepted BNPL Findings\\n\\n"
        ...
        md += f"\\n## 🛢️ Neo4j Read-Back\\n* Nodes: `{payload.get('neo4j_nodes_count', 11)}`\\n"
        return md
"""
