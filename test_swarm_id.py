...
def test_swarm_id():
    ...
    # Now read it back using the query_graph_state method (which should filter by swarm_id and project_scope)
    result = agent.query_graph_state(test_key)
    if result:
        print(f"Read back: {result}")
        # Check that swarm_id and project_scope are in the result
        if result.get('swarm_id') == agent.SWARM_ID and result.get('project_scope') == agent.PROJECT_SCOPE:
            print("SUCCESS: swarm_id and project_scope found in retrieved node.")
        else:
            print("FAILURE: swarm_id or project_scope missing or incorrect.")
            print(f"  Expected swarm_id: {agent.SWARM_ID}, got: {result.get('swarm_id')}")
            print(f"  Expected project_scope: {agent.PROJECT_SCOPE}, got: {result.get('project_scope')}")
    else:
        print("FAILURE: Could not read back the node.")
